        ...
        <v-row v-if="user" no-gutters>
            ...
            <v-col class="col col-12 col-md-8">
                ...
                <v-card class="pa-4 mt-4">
                    ...
                    <ImageInput
                        :background="user.avatar ? `${user.avatar}` : undefined"
                        @change="setImage"
                        :rules="rules"
                    />
                    <PersonalInfo
                    ...
                    <Add
                        v-if="!resume"
                        @click="$router.push(`/job/resumes/create`)"
                        class="w-100"
                    />
                    <ResumeItem
                        :class="{ deleted: resume.is_deleted }"
                        @delete="deleteResume"
                        :editable="true"
                        v-if="resume"
                        :resume="resume"
...
export default {
    head() {
        let descr =
                "Личный кабинет факультета информационных систем и технологий",
    ...
    async created() {
        if (!this.currentUser) this.$router.push("/auth");
        let token = this.$store.getters["auth/getAccessToken"];
        if (this.user.id) {
            this.ideas = (
                await this.$axios.get(`/api/v1/ideas/?author=${this.user.id}`, {
    ...
    methods: {
        ...
        deleteIdea(options) {
            this.$axios
                .delete(`api/v1/ideas/${options.id}/`, {
                    withCredentials: true,
                    headers: {
                        Authorization: `Bearer ${this.$store.getters["auth/getAccessToken"]}`,
                    },
        ...
        setImage(e) {
            ...
            this.$axios
                .patch("api/v1/users/change/avatar/", data, {
                    withCredentials: true,
                    headers: {
                        Authorization: `Bearer ${this.$store.getters["auth/getAccessToken"]}`,
        ...
        savePersonal(user) {
            this.$axios
                ...
                .catch(
                    (res) => (this.personalError = "Произошла какая-то ошибка")
                );
        },
        ...
        changeUsername(username, checkPassword) {
            ...
            this.$axios
                ...
                .catch(
                    (res) =>
                        (this.usernameError =
                            "Возможно это имя занято или ещё какая хуйня на сервере")
                );
        },
