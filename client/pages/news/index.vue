    ...
    <v-container class="mt-4 pa-2">
        ...
        <v-row no-gutters>
            ...
            <v-col class="col col-md-7 col-12">
                <v-text-field placeholder="Поиск новости..." persistent-hint append-icon="mdi-magnify" dense solo
                    v-model="query" />
                <Filters class="d-auto d-md-none" :tags="tags" @applyFilters="applyFilter" />
                <h2 class="text-h5 text--primary">Доступные новости</h2>
                <v-container v-if="filteredNews.length != 0">
                    <NewsItem v-for="(newsItem, index) in filteredNews" :key="index" :item="newsItem" class="my-8" />
                </v-container>
                <v-container v-else>
                    <w-no-info></w-no-info>
                </v-container>
                <v-alert border="bottom" color="pink darken-1" dark v-if="error.length !== 0" class="mt-4">
                    {{ error }}
                </v-alert>
                <div v-show="isLazyLoading" class="text-center">
                    <v-progress-circular indeterminate color="primary" />
                </div>
                <div v-show="!isLazyLoading && !nextExist && filteredNews.length != 0">
                    <h3 class="text-h5 text--primary text-center">Вы просмотрели все новости</h3>
                </div>
                <div v-intersection="addNews" v-show="!isLazyLoading && nextExist && filteredNews.length != 0"
                    class="makeMore">
                    <v-btn @click="addNews" color="primary lighten-2" text small>
                        Смотреть еще...
                    </v-btn>
                </div>
            </v-col>
...
export default {
    ...
    fetch() {
        ...
        this.$axios.get('api/v1/news', { params }).then(res => {
    ...
    computed: {
        filteredNews() {
            let news = this.news.filter(item => {
                ...
                if (item.title.trim().toLowerCase().includes(this.query.trim().toLowerCase())
                    && tagMatch) {
                    return item
                }
            })
