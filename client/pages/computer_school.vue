    ...
    <v-container class="">
        ...
        <v-row no-gutters>
            ...
            <v-col class="col col-12 col-md-8 pa-2"> 
                <h1 class="text-h5 text-md-h4 my-2 text--primary">Направления подготовки</h1>
                <v-breadcrumbs 
                    class="pa-0 pb-2 mb-2" 
                    :items="breadcrumbs"
                />
                <v-container class="pb-2">
                    <CSDirectonItem v-for="(direction, index) in computerSchoolDirections" :key="index" :item="direction"/>
                </v-container>
...
export default {
    async fetch(){
        ...
        this.computerSchoolDirections = await this.$store.getters['cs/getDirections']
    },
    mounted() {
        this.isLoaded = true
    },
