    ...
    <v-container class="mt-4 pa-2">
        ...
        <v-row no-gutters>
            ...
            <v-col class="col col-12 col-md-7">
                ...
                <v-container v-if="filteredLabs.length !== 0" class="pb-2 d-flex flex-wrap justify-space-between">
                    <LabCard class="labs_item mt-4" v-for="(item, index) in filteredLabs" :lab="item" :key="index" />
                </v-container>
                <v-container v-else>
                    <w-no-info></w-no-info>
                </v-container>
                <v-container class="d-flex mb-2 justify-center align-center">
                    ...
                    >Больше</v-btn>
                    <v-btn
                        v-else-if="labs.length == filteredLabs.length && toShow > 4"
                        @click="toShow = 4"
                        color="primary"
                    >Скрыть</v-btn>
                </v-container>
...
export default {
    ...
    head() {
        let descr = 'Текущие лаборатории факультета информационных систем и технологий',
            title = 'Лаборатории',
            type = 'site'
        return {
            title: title,
        ...
        filteredLabs() {
            let labs = this.labs.filter(item => {
                ...
                if ((item.name.trim().toLowerCase().includes(this.query.trim().toLowerCase())
                    || item.description?.trim().toLowerCase().includes(this.query.trim().toLowerCase())
                ) && tagMatch) {
                    return item
                }
            })
