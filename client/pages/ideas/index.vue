<template>
    <v-container class="mt-4 pa-2 d-sm">
        ...
        <v-row no-gutters>
            ...
            <v-col class="col col-12 col-md-7">
                <v-row class="pa-2 mb-2">
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="success lighten-2" small fab nuxt to="ideas/suggest">
                        <v-icon>
                            mdi-plus
                        </v-icon>
                    </v-btn>
        ...
        <v-row no-gutters>
            ...
            <v-col class="col col-12 col-md-7">
                <v-text-field placeholder="Поиск идей..." persistent-hint append-icon="mdi-magnify" dense solo
                    v-model="query" />
                ...
                <v-container v-if="filteredIdeas.length !== 0" class="pb-2 d-flex flex-wrap justify-space-between">
                    <IdeaCard class="labs_item mt-4" v-for="(item, index) in filteredIdeas" :idea="item" :key="index" />
                    <Add @click="$router.push('/ideas/suggest')" class="labs_item mt-4" />
                </v-container>
                <v-container v-else>
                    <w-no-info></w-no-info>
                </v-container>
                <div class="d-flex justify-center">
                    <v-btn v-if="ideas.length > filteredIdeas.length" @click="showMore" color="primary">
                        Больше
                    </v-btn>
                    <v-btn v-else-if="ideas.length === filteredIdeas.length && toShow > 4" @click="toShow = 4"
                        color="primary">
                        Скрыть
                    </v-btn>
                </div>
