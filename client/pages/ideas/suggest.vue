            ...
            <v-col class="col col-12 col-md-8">
                ...
                <v-card>
                    <v-card-text class="pa-2 pa-md-4">
                        <v-container
                            class="image_input mt-4"
                            :class="{
                                redBorder:
                                    $v.idea.image.$dirty &&
                                    $v.idea.image.$invalid,
                            }"
                            @click="$v.idea.image.$touch()"
                        >
                            <ImageInput
                                :background="background"
                                @input="$v.idea.image.$touch()"
                                @blur="$v.idea.image.$touch()"
                                @change="setImage"
                                :rules="rules"
                            ></ImageInput>
                        </v-container>
                        <v-container>
                            ...
                            <v-text-field
                                @input="$v.idea.name.$touch()"
                                @blur="$v.idea.name.$touch()"
                            ...
                            ></v-text-field>
                            <v-textarea
                                @input="$v.idea.description.$touch()"
                                @blur="$v.idea.description.$touch()"
                                :error-messages="descriptionErrors"
                                outlined
                                hint="Эта информация будет в карточке"
                            ...
                            <v-card
                                ...
                                :class="{
                                    redBorder:
                                        $v.idea.information.$dirty &&
                                        $v.idea.information.$invalid,
                                }"
                            >
...
import CreateTeam from "@/components/ideas/CreateTeam";
...
export default {
    ...
    head() {
        let descr =
                "Предложить идею для факультета информационных систем и технологий",
    ...
    computed: {
        ...
        nameErrors() {
            ...
            !this.$v.idea.name.maxLength &&
                errors.push("Слишком длинное название");
            !this.$v.idea.name.minLength &&
                errors.push("Слишком короткое название");
        ...
        descriptionErrors() {
            ...
            !this.$v.idea.description.maxLength &&
                errors.push("Сократите описание");
            !this.$v.idea.description.minLength &&
                errors.push("Описание должно быть содержательно");
            !this.$v.idea.description.required &&
        ...
        informationErrors() {
            ...
            !this.$v.idea.information.minLength &&
                errors.push("Описание должно быть содержательным");
            !this.$v.idea.information.required &&
                errors.push("Описание обязательно");
            return errors;
        },
        tagsErrors() {
        ...
        avatarErrors() {
            const errors = [];
            if (!this.$v.idea.image.$dirty) return errors;
            !this.$v.idea.image.required && errors.push("Добавьте фото");
            return errors;
        },
    },
    watch: {
