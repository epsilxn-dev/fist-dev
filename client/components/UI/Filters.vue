        ...
        <v-card-text>
            ...
            >
                <template v-slot:no-data>
                    <v-list-item>
                        <v-list-item-title> Тега не найдено </v-list-item-title>
                    </v-list-item>
                </template>
            ...
            <v-radio-group v-model="reverse">
                <v-radio :label="'Сначала новые'" :value="false"></v-radio>
                <v-radio :label="'Сначала старые'" :value="true"> </v-radio>
            </v-radio-group>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn @click="applyFilters" color="primary" small> Найти </v-btn>
        </v-card-actions>
    </v-card>
</template>
...
export default {
    ...
    methods: {
        applyFilters() {
            let filters = {};
            filters.selectedTag = this.tags[this.selectedTag]?.name
                ? this.tags[this.selectedTag].name
                : "";
    ...
    watch: {
        ...
        selectedTag(val) {
            let filters = {};
            filters.selectedTag = this.tags[val]?.name
                ? this.tags[val].name
                : "";
            filters.reverse = this.reverse;
            this.$emit("applyFilters", filters);
